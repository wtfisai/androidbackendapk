...
router.get('/screen-info', authenticateApiKey, async (req, res) => {
  try {
    ...
    // Get display info
    const { stdout: displayInfo } = await execAsync(
      'dumpsys display | grep "mDisplayWidth\\|mDisplayHeight\\|orientation"'
    );
    ...
    const screenInfo = {
      ...
      orientation:
        displayInfo.includes('orientation=1') || displayInfo.includes('orientation=3')
          ? 'landscape'
          : 'portrait',
      timestamp: new Date()
    };
...
router.post('/rotate', authenticateApiKey, async (req, res) => {
  try {
    ...
    const rotation =
      orientationMap[orientation] !== undefined ? orientationMap[orientation] : orientation;
